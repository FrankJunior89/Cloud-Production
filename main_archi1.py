...
df4 = df.filter((hour(col("Date")) >= 22) | (hour(col("Date")) <= 4)).groupBy("Community Area").agg(count("*").alias("TotalCrimes")).orderBy(col("TotalCrimes"))
...
df4.coalesce(1).write.csv(output_csv_path4, header=True, mode="overwrite")
